...
const ErrorModal: React.FC<{ isOpen: boolean; onClose: () => void; errorData: any }> = ({ isOpen, onClose, errorData }) => {
  ...
  const steps = [
    { id: 'BROWSER', label: 'Browser', status: 'success', data: trace.payloadSent || { info: "Richiesta avviata" } },
    ...
    { id: 'N8N', label: 'n8n Server', status: (trace.step === 'N8N_SERVER' || trace.step === 'N8N_SERVER_TIMEOUT') ? 'error' : (trace.step === 'NETWORK_FAILURE' ? 'warning' : 'pending'), data: { url: trace.targetUrl || 'Non raggiunto', response: trace.rawResponse || errorData?.details || "Nessuna risposta" } }
  ];
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden border border-slate-200 my-8">
        <div className="bg-red-600 px-8 py-6 flex justify-between items-center text-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-black uppercase tracking-widest text-sm">Trace di Errore</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-3">
              ...
              <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 font-mono text-[11px] text-emerald-400 overflow-x-auto shadow-inner h-48 no-scrollbar">
                <pre>{JSON.stringify(trace.payloadSent || { info: "In attesa..." }, null, 2)}</pre>
              </div>
            </div>
            <div className="space-y-3">
              <p className="text-[11px] text-slate-500 font-black uppercase tracking-widest flex items-center gap-2">
        ...
        <div className="bg-slate-50 px-8 py-6 border-t border-slate-100 flex justify-end gap-3">
          <button onClick={() => window.location.reload()} className="px-6 py-3 bg-white border border-slate-200 text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-100 transition-all">Reload</button>
          <button onClick={onClose} className="px-10 py-3 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-black transition-all">Chiudi</button>
        </div>
      </div>
    </div>
  );
...
const App: React.FC = () => {
  ...
  const handleRefresh = useCallback(async (isInitial = false, isBackground = false) => {
    ...
    try {
      ...
      setErrorDetails({ message: error.message, trace: { step: 'INTERNAL_ERROR' } });
      setIsErrorModalOpen(true);
    } finally {
      ...
      <main className="max-w-[1400px] mx-auto px-4 sm:px-8">
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          <div className="flex-1 w-full space-y-6">
            ...
            <section className="bg-white rounded-3xl p-8 card-shadow flex items-center justify-between border border-slate-100 h-36">
               <div className="flex items-center gap-10">
                  <div className="flex items-baseline gap-3">
                    <span className="text-5xl font-black text-slate-900">{items.length}</span>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">News</span>
                  </div>
                  <div className="w-px h-12 bg-slate-100"></div>
                  <div className="flex flex-col">
                    <span className="text-xl font-black text-slate-900">Live Status</span>
                    <span className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${isLive ? 'text-emerald-500' : 'text-amber-500'}`}>
                      ...
                      {isLive ? 'Connesso' : 'Offline'}
                    </span>
                  </div>
               </div>
               <button onClick={() => handleRefresh()} disabled={isRefreshing} className="w-16 h-16 bg-slate-900 text-white rounded-2xl flex items-center justify-center hover:bg-black transition-all shadow-xl">
            ...
            <section className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 card-shadow border border-white/50 space-y-6">
              <div className="relative w-full">
                <input 
                  ...
                  className="w-full h-16 pl-16 pr-6 bg-slate-100/50 rounded-2xl text-lg font-medium focus:outline-none focus:ring-4 focus:ring-slate-900/5 border border-slate-200/50"
                />
                ...
                {CATEGORIES.map(cat => (
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all border-2 ${
                      activeTag === cat ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-400 border-slate-100 hover:border-slate-200'
                    }`}
                  >
                    {cat}
                  </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {status === AppState.LOADING ? (
                [1,2,3,4].map(i => <div key={i} className="h-64 bg-white/40 animate-pulse rounded-3xl"></div>)
          ...
          <aside className="w-full lg:w-[400px]">
            <div className="bg-[#0a0a0a] rounded-[2.5rem] p-10 text-white min-h-[700px] shadow-2xl sticky top-8 border border-white/5">
              <h2 className="text-3xl font-black mb-8">News Flow</h2>
                ...
                {items.slice(0, 15).map((item, i) => (
                  <div key={item.id} className="flex gap-5 items-center group">
                    ...
                    <div className="flex-1 truncate">
                      <p className="font-bold text-sm truncate group-hover:text-white transition-colors">{item.title}</p>
